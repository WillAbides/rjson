...
above and again by the state machine. Another exception is strings that contain escape sequences. I haven't found a safe
way around that.
...
- **drop in replacement for "encoding/json"** - rjson doesn't aim to be a replacement for "encoding/json". It 
  probably won't ever unmarshal json to a struct, and it definitely won't ever marshal json. It does one thing well, 
  and that one thing is parsing json fast and with minimal memory allocations.

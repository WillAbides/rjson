...
func readInt32Helper(neg bool, digits []byte) (int32, error) {
	const cutoff = uint32(1 << uint32(31))
	un, err := readUint32Helper(digits)
	if err != nil {
		return 0, err
	}
	if neg {
...
func readUint32Helper(data []byte) (uint32, error) {
	val := uint32(0)
